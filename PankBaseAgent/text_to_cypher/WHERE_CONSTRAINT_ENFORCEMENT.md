...
1. **PlannerAgent** sending broad queries: "Get all SNPs that have QTL_for relationships"
...
Query: 'Get SNPs that have QTL_for relationships with gene CFTR'
MATCH (sn:snp)-[r:QTL_for]->(g:gene) WHERE g.name = 'CFTR'
...
4. "Get SNPs that have QTL_for relationships with gene CFTR"  ← Specific gene
...
- Example: "How does SNP rs2402203 contribute to CFTR's function in T1D?"
  ...
  - ✅ GOOD: "Find SNP rs2402203" → "Find gene CFTR" → "Get SNPs with QTL_for relationships to gene CFTR"
...
**PlannerAgent sends**:
```
"Get all SNPs that have QTL_for relationships with genes"
```
...
**PlannerAgent sends**:
```
"Get SNPs that have QTL_for relationships with gene CFTR"
```
