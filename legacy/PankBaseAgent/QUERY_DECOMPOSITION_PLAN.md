...
│   └─ Requires: gene lookup + part_of_QTL_signal relationship + property retrieval
...
│   └─ Requires: gene lookup + disease context + part_of_QTL_signal + PIP filtering + credible set analysis
...
```json
{
  "functions": [
    {"name": "pankbase_api_query", "input": "Find gene with name CFTR"},
    {"name": "pankbase_api_query", "input": "Get all SNPs that have part_of_QTL_signal relationships with genes"}
  ]
}
```
