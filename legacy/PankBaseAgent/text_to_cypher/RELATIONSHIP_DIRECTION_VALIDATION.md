...
```
**Problem**: `part_of_QTL_signal` should go from `snp` to `gene`, not the other way around.
...
| `part_of_QTL_signal` | snp | gene | `(sn:snp)-[q:part_of_QTL_signal]->(g:gene)` |
...
**Note**: 
- `part_of_QTL_signal`: snp → gene ✅ Correct
