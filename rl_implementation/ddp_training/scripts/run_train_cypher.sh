#!/bin/bash
# =============================================================================
# Run Cypher Generator Training from Rollouts
#
# Trains the Cypher Generator model using pre-collected rollouts.
# Uses GRPO (Group Relative Policy Optimization) for advantage estimation.
#
# Prerequisites:
#   - Rollouts collected via collect_rollouts.py
#   - Rollouts stored in JSONL format
#
# Usage:
#   ./run_train_cypher.sh [config_file]
#   ./run_train_cypher.sh --rollouts /path/to/rollouts.jsonl --num-epochs 10
#
# Examples:
#   ./run_train_cypher.sh                                    # Use default config
#   ./run_train_cypher.sh ../config/train_cypher_config.yaml # Custom config
#   ./run_train_cypher.sh --rollouts my_rollouts.jsonl       # Override rollouts
# =============================================================================

set -e

echo "=============================================="
echo "Cypher Generator Training from Rollouts"
echo "=============================================="
...
# Default config file
DEFAULT_CONFIG="$CONFIG_DIR/train_cypher_config.yaml"

# Default paths
DEFAULT_ROLLOUTS="$PROJECT_DIR/outputs/stage1_ddp/rollouts_collect_base_model.jsonl"
DEFAULT_CYPHER_MODEL="$PROJECT_DIR/models/qwen2.5-coder-14b"
DEFAULT_OUTPUT_DIR="$PROJECT_DIR/outputs/cypher_trained"
...
# Build command
CMD="python3 train_cypher_from_rollouts.py"
