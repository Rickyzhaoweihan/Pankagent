...
and trains the Cypher Generator model using PPO/GRPO.

Usage:
    python train_cypher_from_rollouts.py --config config/train_cypher_config.yaml
    python train_cypher_from_rollouts.py --rollouts /path/to/rollouts.jsonl --cypher-model /path/to/model
...
from rollout_loader import RolloutLoader, CypherBatchPreparer
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train Cypher Generator from rollouts")
    ...
    # Model
    parser.add_argument("--cypher-model", type=str, default=None,
                        help="Path to Cypher Generator model")
    ...
    # Schema and Experience Buffer (for prompt rebuilding)
    parser.add_argument("--schema-path", type=str, default=None,
                        help="Path to KG schema JSON file (only needed if --rebuild-prompts)")
    parser.add_argument("--experience-buffer-path", type=str, default=None,
                        help="Path to experience buffer JSON file (optional)")
    parser.add_argument("--rebuild-prompts", action="store_true",
                        help="Rebuild prompts instead of using stored ones (for old rollouts with truncated prompts)")
...
def merge_config(config: Dict[str, Any], args) -> Dict[str, Any]:
    ...
    if args.cypher_model:
        config['cypher_model_path'] = args.cypher_model
    ...
    # Schema and experience buffer for prompt rebuilding
    if args.schema_path:
        config['schema_path'] = args.schema_path
    if args.experience_buffer_path:
        config['experience_buffer_path'] = args.experience_buffer_path
    if args.rebuild_prompts:
        config['rebuild_prompts'] = True
    
    return config


class CypherTrainer:
    """Trainer for Cypher Generator from rollouts."""
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.batch_size = config.get('batch_size', 2)  # Default to 2 for memory
        self.save_frequency = config.get('save_frequency', 1)
        
        # Initialize components
        logger.info("=" * 60)
        logger.info("CYPHER GENERATOR TRAINING FROM ROLLOUTS")
        logger.info("=" * 60)
    ...
    def _init_rollout_loader(self):
        ...
        rewards = [r.cypher_reward for r in self.rollouts]
        if rewards:
            low_reward = sum(1 for r in rewards if r < 0.3)
            med_reward = sum(1 for r in rewards if 0.3 <= r < 0.7)
            high_reward = sum(1 for r in rewards if r >= 0.7)
            logger.info(f"  Reward distribution: low={low_reward}, med={med_reward}, high={high_reward}")
            logger.info(f"  Avg reward (filtered): {sum(rewards)/len(rewards):.3f}")
    
    def _init_ddp_trainer(self):
        """Initialize DDP or FSDP trainer for Cypher Generator."""
        ...
        samples_per_epoch = sum(len(r.steps) for r in self.rollouts)
    ...
    def _init_batch_preparer(self):
        ...
        # Load experience buffer if path provided
        experience_buffer = None
        experience_buffer_path = self.config.get('experience_buffer_path')
        if experience_buffer_path:
            try:
                from agents.experience_buffer import ExperienceBuffer
                experience_buffer = ExperienceBuffer(persist_path=experience_buffer_path)
                logger.info(f"  Loaded experience buffer from {experience_buffer_path}")
            except Exception as e:
                logger.warning(f"  Failed to load experience buffer: {e}")
        
        # Get schema path (default to project schema)
        schema_path = self.config.get('schema_path')
        if schema_path is None:
            # Use default schema path
            schema_path = str(PROJECT_DIR / "legacy/PankBaseAgent/text_to_cypher/data/input/kg_schema.json")
            if not Path(schema_path).exists():
                logger.warning(f"  Default schema not found at {schema_path}")
                schema_path = None
        
        # Default to using stored prompts (since we now store full prompts)
        # Set to True only for old rollouts with truncated prompts
        self.rebuild_prompts = self.config.get('rebuild_prompts', False)
        
        self.batch_preparer = CypherBatchPreparer(
        ...
        if schema_path and self.rebuild_prompts:
            logger.info(f"  Batch preparer initialized with prompt rebuilding enabled")
    ...
    def train(self):
        ...
        for epoch in range(self.num_epochs):
            ...
            # Track best - only save when we improve
            ...
            current_loss = epoch_metrics.get('policy_loss', float('inf'))
            should_save = current_loss < self.best_loss
            ...
            if should_save:
                ...
                logger.info(f"🏆 New best model! Loss: {current_loss:.4f}")
    ...
    def train_epoch(self) -> Dict[str, float]:
        ...
        1. Prepare full batch from rollouts
        2. Compute old_log_probs ONCE
        3. Compute advantages using GRPO
        4. ONE ppo_updater.update_step() - mini-batching handled inside ddp_trainer.train_step()
        """
        # Prepare full batch from all rollouts
        ...
        if use_balanced and dist.is_initialized():
            local_rank = int(os.environ.get('LOCAL_RANK', 0))
            if local_rank == 0:
        ...
        try:
            # Use balanced sampling if configured (CRITICAL for bimodal reward distribution!)
            batch = self.batch_preparer.prepare_batch(
                self.rollouts,
                use_all_steps=True,
                rebuild_prompts=self.rebuild_prompts,
        ...
        if dist.is_initialized():
            ...
            if error_tensor.item() > 0:
                logger.error("Batch preparation failed on one or more ranks, skipping epoch")
                return {'error': 1.0}
        elif batch_error:
            return {'error': 1.0}
        ...
        # This is exactly how train_stage1.py does it
        logger.info("Preparing PPO batch with advantages...")
        ...
        epoch_metrics = {
            ...
            'learning_rate': update_metrics.get('learning_rate', 0.0),
    ...
    def _create_merged_model(self, adapter_path: str, output_path: str):
        ...
        base_model_path = self.config['cypher_model_path']
...
def main():
    ...
    required = ['rollouts_path', 'cypher_model_path', 'checkpoint_dir']
